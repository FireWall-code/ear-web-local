...
async function connectSPP(sppPort=null) {
    ...
    ws.onmessage = (event) => {
        ...
        if (rawData[0] !== 85 || rawData.length < 10) return;

        let header = rawData.slice(0, 6);
        let command = getCommand(header);
        const hex = rawData.reduce((acc, byte) => acc + byte.toString(16).padStart(2, '0'), '');

        if (command === 57345 || command===16391) readBattery(hex);
        if (command === 57347) readANC(hex);
        if (command === 16452) readCustomEQ(rawData);
        if (command === 16415 || command === 16464) readEQ(hex);
        if (command === 16450) readFirmware(hex);
        if (command === 57357) readEarFitTestResult(hex);
        if (command === 16416) readPersonalizedANC(rawData);
        if (command === 16398) readInEar(hex);
        if (command === 16449) readLatency(hex);
        if (command === 16407) readLEDCaseColor(hex);
        if (command === 16408) readGesture(hex);
        if (command === 16414) readANC(hex);
        if (command === 16460) read_advanced_eq_status(hex);
        if (command === 16462) read_enhanced_bass(hex);
    ...
    ws.onerror = () => {
        console.error('Bridge non disponible. Lancez: node bridge.js');
    };
}
