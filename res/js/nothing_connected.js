...
function setStatus(msg) {
    const el = document.getElementById("device_container");
    if (el) el.innerHTML = '<div style="padding:10px;text-align:center">' + msg + '</div>';
    const btn = document.getElementById("scan_button-c");
...
async function scanNewDevicesSerial() {
    ...
    ws.onmessage = (event) => {
        ...
        if (typeof data === 'string') {
            try {
                ...
                if (msg.type === 'connected') {
                    setStatus("Connecté, identification...");
                    requestSerialNumber();
                } else if (msg.type === 'connecting') {
                    setStatus("Connexion Bluetooth en cours...");
                } else if (msg.type === 'disconnected') {
                    setStatus("Bluetooth déconnecté");
        ...
        if (command === 16390) {
            let serialNum = getSerialNumber(hex);
            console.log(serialNum);
            processSerial(serialNum);
        }
        if (command === 16450) {
            let firmwareVersion = readFirmwareFromData(hex);
    ...
    ws.onerror = () => {
        setStatus("Bridge non disponible. Lancez: node bridge.js");
    };
}